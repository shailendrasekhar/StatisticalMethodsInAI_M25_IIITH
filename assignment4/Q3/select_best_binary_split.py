...
def select_best_binary_split(x_NF, y_N, MIN_SAMPLES_LEAF=1):
    ...
    for f in range(F):
        ...
        # Initialize lists to store results for each possible threshold
        left_cost_V_list = []
        right_cost_V_list = []
        left_yhat_V_list = []
        right_yhat_V_list = []

        # Loop over all possible thresholds for the current feature 'f'
        for thresh in possib_xthresh_V:
            # Split the data and labels into left and right children
            ...
            # The prediction for a regression leaf is the mean of the labels
            mean_y_L = np.mean(y_L) if y_L.size > 0 else 0
            mean_y_R = np.mean(y_R) if y_R.size > 0 else 0
            
            left_yhat_V_list.append(mean_y_L)
            right_yhat_V_list.append(mean_y_R)
            
            # The cost is the Sum of Squared Errors (SSE) from the mean
            left_cost = np.sum(np.square(y_L - mean_y_L))
            right_cost = np.sum(np.square(y_R - mean_y_R))

            left_cost_V_list.append(left_cost)
            right_cost_V_list.append(right_cost)

        left_yhat_V = np.array(left_yhat_V_list)
        right_yhat_V = np.array(right_yhat_V_list)
        left_cost_V = np.array(left_cost_V_list)
        right_cost_V = np.array(right_cost_V_list)
